...
const parseRawTraceEvent = (input: unknown): ParseResult<RawTraceEvent> => {
  ...
  const eventTypeResult: ParseResult<string> = parseNonEmptyString(eventRecord.eventType, 'event.eventType');
  const timestampResult: ParseResult<string> = parseNonEmptyString(eventRecord.timestamp, 'event.timestamp');
  const sourceResult: ParseResult<string> = parseNonEmptyString(eventRecord.source, 'event.source');
  const runIdResult: ParseResult<string | undefined> = parseOptionalNonEmptyString(eventRecord.runId, 'event.runId');
  ...
  const issues: ParseIssue[] = combineIssues(idResult, eventTypeResult, timestampResult, sourceResult, runIdResult, correlationIdResult);
  if (issues.length > 0) {
...
export const convertRawTraceEventToTraceEvent = (raw: RawTraceEvent): ParseResult<TraceEvent> => {
  ...
  const correlationId = raw.correlationId === undefined ? undefined : toBrand<'CorrelationId'>(raw.correlationId);
