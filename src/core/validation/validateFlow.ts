...
import { filterEventsByScope, type EventScope, type ValidationReport } from './models/validationReport';
import { evaluateRule } from './evaluators/evaluateRule';

export const validateFlow = (flow: FlowDefinition, events: TraceEvent[], eventScope?: EventScope): ValidationReport => {
  ...
  const violations: RuleViolation[] = ruleEvaluations.flatMap((evaluation: RuleEvaluation): RuleViolation[] => evaluation.violations);

  const failedRuleCount: number = ruleEvaluations.filter((evaluation: RuleEvaluation): boolean => !evaluation.passed).length;
